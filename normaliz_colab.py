...
Original file is located at
    https://colab.research.google.com/drive/1ynFRpYHyU1r0Saauk_OeEntbBlIFEhfy
"""
...
def noslangize(str):
  if len(str) > 2 and str in slangdict:
      return slangdict.get(str)
  else:
    return str
...
def removedup(str):
  ret_str = ""
  for ch in str:
    if len(ret_str) <= 1 or ret_str[-2] !=  ch or ret_str[-1] != ch:
      ret_str += ch
  return ret_str
...
def preprocess(str):
  ...
  for i in range(len(tokens)):
    tokens[i] = americanize(removedup(tokens[i]))
  return tokens

def lexical(tokens):
  for i in range(len(tokens)):
    tokens[i] = noslangize(tokens[i])
  return tokens


inp = "yayyyy bro i was rofl on da floor it was sic and normalising af colour"
print(lexical(preprocess(inp)))